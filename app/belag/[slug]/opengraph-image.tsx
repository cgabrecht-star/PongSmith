...
import { loadRubberBySlug } from "@/lib/product-detail";

export const alt = "PongSmith Belag-Detail";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

export default async function OgImage({ params }: { params: { slug: string } }) {
  const product = await loadRubberBySlug(params.slug);
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, color: "#fafaf7", fontSize: 22, letterSpacing: "0.06em", fontWeight: 700, textTransform: "uppercase" }}>
          ...
          PongSmith
          <span style={{ fontSize: 11, letterSpacing: "0.18em", color: "#ff8b56", marginLeft: 12 }}>BELAG</span>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", marginTop: "auto", marginBottom: 30, gap: 18 }}>
          ...
          {product && (
            <div style={{ display: "flex", gap: 40, marginTop: 24 }}>
              {product.speed !== null && (
                <SpecBadge label="SPEED" value={product.speed.toFixed(1)} />
              )}
              {product.spin !== null && (
                <SpecBadge label="SPIN" value={product.spin.toFixed(1)} />
              )}
              {product.control !== null && (
                <SpecBadge label="CONTROL" value={product.control.toFixed(1)} />
              )}
            </div>
          )}
        </div>
...
function SpecBadge({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <span style={{ color: "#fafaf7", fontSize: 36, fontWeight: 700 }}>{value}<span style={{ color: "#666", fontSize: 18 }}>/10</span></span>
    </div>
  );
}
